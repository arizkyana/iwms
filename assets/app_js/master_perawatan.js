...
function setTable(){
	...
	$('#tb_list').DataTable( {
		...
		"serverSide": true,
		ajax: {
			'url':base_url+"Master_perawatan/ajax_list",
			'type':'POST'			
		}
    });
}
...
function validateForm(){
	...
	if($('hid_id').val()==''){
		...
		$.ajax({
			...
			type:"POST",
			url:base_url+"Master_perawatan/cek_nopol/"+nopol,
			dataType:"html",
...
function simpanData(){
	...
	$("#form_editing").ajaxSubmit({
		url:base_url+"Master_perawatan/simpan_data",
		type: 'post',
		success: function(){
...
function editData(id){
	...
	$.ajax({
		...
		type:"POST",
		url:base_url+"Master_perawatan/get_fleet_data/"+id,
		dataType:"html",
		success:function(data){
	...
	bootbox.confirm("Anda yakin akan menghapus data ini ?",
		function(result){
			if(result==true){
				...
				$.post(
					base_url+"Master_perawatan/delete_data/"+id,function(){
