...
function setTable(){
	...
	$('#tb_list').DataTable( {
		...
		"serverSide": true,
		ajax: {
			'url':base_url+"Master_perencanaan/ajax_list",
			'type':'POST'			
		}
    });
}
...
function validateForm(){
	...
	if($('hid_id').val()==''){
		...
		$.ajax({
			...
			type:"POST",
			url:base_url+"Master_perencanaan/cek_nopol/"+nopol,
			dataType:"html",
...
function simpanData(){
	...
	$("#form_editing").ajaxSubmit({
		url:base_url+"Master_perencanaan/simpan_data",
		type: 'post',
		success: function(){
...
function editData(id){
	...
	$.ajax({
		...
		type:"POST",
		url:base_url+"Master_perencanaan/get_fleet_data/"+id,
		dataType:"html",
		success:function(data){
	...
	bootbox.confirm("Anda yakin akan menghapus data ini ?",
		function(result){
			if(result==true){
				...
				$.post(
					base_url+"Master_perencanaan/delete_data/"+id,function(){
