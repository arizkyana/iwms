...
function setTable(){
	...
	$('#tb_list').DataTable( {
		...
		"serverSide": true,
		ajax: {
			'url':base_url+"Master_rumah/ajax_list",
			'type':'POST'			
		}
    });
}
...
function validateForm(){
	...
	if($('hid_id').val()==''){
		...
		//cek id_rumah di database
		$.ajax({

			type:"POST",
			url:base_url+"Master_rumah/cek_id_rumah/"+id_rumah,
			dataType:"html",
...
function simpanData(){
	...
	$("#form_editing").ajaxSubmit({
		url:base_url+"Master_rumah/simpan_data",
		type: 'post',
		success: function(){
...
function editData(id){
	...
	$.ajax({
		...
		type:"POST",
		url:base_url+"Master_rumah/get_fleet_data/"+id,
		dataType:"html",
		success:function(data){
			...
			$('#txt_id_rumah').val(data['id_rumah']);
	...
	bootbox.confirm("Anda yakin akan menghapus data ini ?",
		function(result){
			if(result==true){
				...
				$.post(
					base_url+"Master_rumah/delete_data/"+id,function(){
