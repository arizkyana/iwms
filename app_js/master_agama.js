...
function setTable(){
	...
	$('#tb_list').DataTable( {
		...
		"serverSide": true,
		ajax: {
			'url':base_url+"Master_agama/ajax_list",
			'type':'POST'			
		}
    });
}
...
function validateForm(){
	...
	if($('hid_id').val()==''){
		...
		//cek agama di database
		$.ajax({

			type:"POST",
			url:base_url+"Master_agama/cek_agama/"+agama,
			dataType:"html",
...
function simpanData(){
	...
	$("#form_editing").ajaxSubmit({
		url:base_url+"Master_agama/simpan_data",
		type: 'post',
		success: function(){
...
function editData(id){
	...
	$.ajax({
		...
		type:"POST",
		url:base_url+"Master_agama/get_agama_data/"+id,
		dataType:"html",
		success:function(data){
			...
			$('#txt_nmagama').val(data['nama_agama']);
	...
	bootbox.confirm("Anda yakin akan menghapus data ini ?",
		function(result){
			if(result==true){
				...
				$.post(
					base_url+"Master_agama/delete_data/"+id,function(){
